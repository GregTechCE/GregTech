...
public class GT_GUIContainer_MultiMachine extends GT_GUIContainerMetaTile_Machine {
    ...
    public GT_GUIContainer_MultiMachine(InventoryPlayer aInventoryPlayer, IGregTechTileEntity aTileEntity, String aName, String aTextureFile) {
        super(new ContainerMultiMachine(aInventoryPlayer, aTileEntity), RES_PATH_GUI + "multimachines/" + (aTextureFile == null ? "MultiblockDisplay" : aTextureFile));
        mName = aName;
    }
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        if (mContainer != null) {
            if ((((ContainerMultiMachine) mContainer).mDisplayErrorCode & 1) != 0)
                fontRendererObj.drawString("Pipe is loose.", 10, 16, 16448255);
            if ((((ContainerMultiMachine) mContainer).mDisplayErrorCode & 2) != 0)
                fontRendererObj.drawString("Screws are missing.", 10, 24, 16448255);
            if ((((ContainerMultiMachine) mContainer).mDisplayErrorCode & 4) != 0)
                fontRendererObj.drawString("Something is stuck.", 10, 32, 16448255);
            if ((((ContainerMultiMachine) mContainer).mDisplayErrorCode & 8) != 0)
                fontRendererObj.drawString("Platings are dented.", 10, 40, 16448255);
            if ((((ContainerMultiMachine) mContainer).mDisplayErrorCode & 16) != 0)
                fontRendererObj.drawString("Circuitry burned out.", 10, 48, 16448255);
            if ((((ContainerMultiMachine) mContainer).mDisplayErrorCode & 32) != 0)
                ...
                if (((ContainerMultiMachine) mContainer).mActive == 0) {
                   ...
                   if(id == 1157 || id == 1158){
                    	...
                    	if(tItem==null || !GT_Utility.areStacksEqual(tItem, GT_ModHandler.getIC2Item(BlockName.mining_pipe, BlockMiningPipe.MiningPipeType.pipe, 1))){
                    		fontRendererObj.drawString("Missing Mining Pipe", 10,((ContainerMultiMachine) mContainer).mActive == 0 ? 40 : 24, 16448255);
                    	}
