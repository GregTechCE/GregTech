...
public final class ItemLayerModel implements IRetexturableModel {
    public static final net.minecraftforge.client.model.ItemLayerModel INSTANCE = new net.minecraftforge.client.model.ItemLayerModel(ImmutableList.<ResourceLocation>of());

    private final ImmutableList<ResourceLocation> textures;
    private final ItemOverrideList overrides;
    private final int rgbColor;

    public ItemLayerModel(ImmutableList<ResourceLocation> textures, int rgbColor) {
        this(textures, ItemOverrideList.NONE, rgbColor);
    }

    public ItemLayerModel(ImmutableList<ResourceLocation> textures, ItemOverrideList overrides, int rgbColor) {
    ...
    public net.minecraftforge.client.model.ItemLayerModel retexture(ImmutableMap<String, String> textures) {
        ...
        return new net.minecraftforge.client.model.ItemLayerModel(builder.build(), overrides);
    }
    ...
    public IBakedModel bake(IModelState state, final VertexFormat format, Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter) {
        ...
        for (int i = 0; i < textures.size(); i++) {
            ...
            builder.addAll(getQuadsForSprite(i, sprite, format, transform, rgbColor));
        }
        TextureAtlasSprite particle = bakedTextureGetter.apply(textures.isEmpty() ? new ResourceLocation("missingno") : textures.get(0));
        ImmutableMap<TransformType, TRSRTransformation> map = IPerspectiveAwareModel.MapWrapper.getTransforms(state);
        return new ItemLayerModel.BakedItemModel(builder.build(), particle, map, overrides, null);
    }
    ...
    private static final class BakedItemModel implements IPerspectiveAwareModel {
        ...
        public BakedItemModel(ImmutableList<BakedQuad> quads, TextureAtlasSprite particle, ImmutableMap<TransformType, TRSRTransformation> transforms, ItemOverrideList overrides, IBakedModel otherModel) {
            ...
            if (otherModel != null) {
                ...
                this.otherModel = new ItemLayerModel.BakedItemModel(builder.build(), particle, transforms, overrides, this);
                isCulled = false;
            }
        }
    ...
    public static ImmutableList<BakedQuad> getQuadsForSprite(int tint, TextureAtlasSprite sprite, VertexFormat format, Optional<TRSRTransformation> transform, int rgbColor) {
        ImmutableList.Builder<BakedQuad> builder = ImmutableList.builder();
        ...
        for (int f = 0; f < sprite.getFrameCount(); f++) {
            ...
            for (int v = 0; v < vMax; v++) {
                ...
                for (int u = 0; u < uMax; u++) {
                    ...
                    {
                        addSideQuad(builder, faces, format, transform, EnumFacing.WEST, tint, sprite, uMax, vMax, u, v, rgbColor);
                    }
                    if (!ptu && t) // left - opaque, right - transparent
                    {
                        addSideQuad(builder, faces, format, transform, EnumFacing.EAST, tint, sprite, uMax, vMax, u, v, rgbColor);
                    }
                    if (ptv[u] && !t) // up - transparent, down - opaque
                    {
                        addSideQuad(builder, faces, format, transform, EnumFacing.UP, tint, sprite, uMax, vMax, u, v, rgbColor);
                    }
                    if (!ptv[u] && t) // up - opaque, down - transparent
                    {
                        addSideQuad(builder, faces, format, transform, EnumFacing.DOWN, tint, sprite, uMax, vMax, u, v, rgbColor);
                    }
                    ptu = t;
                    ptv[u] = t;
                }
                if (!ptu) // last - opaque
                {
                    addSideQuad(builder, faces, format, transform, EnumFacing.EAST, tint, sprite, uMax, vMax, uMax, v, rgbColor);
                }
            }
            // last line
            for (int u = 0; u < uMax; u++) {
                if (!ptv[u]) {
                    addSideQuad(builder, faces, format, transform, EnumFacing.DOWN, tint, sprite, uMax, vMax, u, vMax, rgbColor);
                }
            }
        }
        // front
        builder.add(buildQuad(format, transform, EnumFacing.NORTH, sprite, tint,
                ...
                1, 0, 7.5f / 16f, sprite.getMaxU(), sprite.getMaxV(), rgbColor
        ));
        // back
        builder.add(buildQuad(format, transform, EnumFacing.SOUTH, sprite, tint,
                ...
                0, 1, 8.5f / 16f, sprite.getMinU(), sprite.getMinV(), rgbColor
        ));
    ...
    private static void addSideQuad(ImmutableList.Builder<BakedQuad> builder, BitSet faces, VertexFormat format, Optional<TRSRTransformation> transform, EnumFacing side, int tint, TextureAtlasSprite sprite, int uMax, int vMax, int u, int v, int rgbColor) {
        int si = side.ordinal();
        if (si > 4) si -= 2;
        int index = (vMax + 1) * ((uMax + 1) * si + u) + v;
        if (!faces.get(index)) {
            faces.set(index);
            builder.add(buildSideQuad(format, transform, side, tint, sprite, u, v, rgbColor));
        }
    }

    private static BakedQuad buildSideQuad(VertexFormat format, Optional<TRSRTransformation> transform, EnumFacing side, int tint, TextureAtlasSprite sprite, int u, int v, int rgbColor) {
    ...
    private static BakedQuad buildQuad(
            ...
            float x3, float y3, float z3, float u3, float v3, int rgbColor) {
        UnpackedBakedQuad.Builder builder = new UnpackedBakedQuad.Builder(format);
        builder.setQuadTint(tint);
        builder.setQuadOrientation(side);
        builder.setTexture(sprite);
        putVertex(builder, format, transform, side, x0, y0, z0, u0, v0, rgbColor);
        putVertex(builder, format, transform, side, x1, y1, z1, u1, v1, rgbColor);
        putVertex(builder, format, transform, side, x2, y2, z2, u2, v2, rgbColor);
        putVertex(builder, format, transform, side, x3, y3, z3, u3, v3, rgbColor);
        return builder.build();
    }

    private static void putVertex(UnpackedBakedQuad.Builder builder, VertexFormat format, Optional<TRSRTransformation> transform, EnumFacing side, float x, float y, float z, float u, float v, int rgbColor) {
        ...
        for (int e = 0; e < format.getElementCount(); e++) {
            switch (format.getElement(e).getUsage()) {
                case POSITION:
                    ...
                    break;
                case COLOR:
                    float red = ((rgbColor >> 16) & 0xFF) / 255.0f;
                    float green = ((rgbColor >> 8) & 0xFF) / 255.0f;
                    float blue = ((rgbColor) & 0xFF) / 255.0f;
