        ...
        switch (uEntry.orePrefix) {
            case gem:
                ...
                if (!OrePrefix.block.isIgnored(uEntry.material)) {
                    ModHandler.addCompressionRecipe(GTUtility.copyAmount(9, stack), OreDictionaryUnifier.get(OrePrefix.block, uEntry.material));
                }
                if (!aNoSmelting && uEntry.material instanceof MetalMaterial) {
                    ModHandler.addSmeltingRecipe(GTUtility.copyAmount(1, stack), OreDictionaryUnifier.get(OrePrefix.ingot, ((MetalMaterial)uEntry.material).smeltInto, 1));
                }
                if (aNoSmashing) {
