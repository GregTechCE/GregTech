            ...
            case cell:
                if (uEntry.material == Materials.Empty) {
                    ...
                    if ((uEntry.material.mMaterialList.size() > 0) && ((uEntry.material.mExtraData & 0x3) != 0)) {
                        ...
                        tItemAmount = (tItemAmount * tDensityMultiplier % uEntry.material.getDensity() > 0L ? 1 : 0) + tItemAmount * tDensityMultiplier / uEntry.material.getDensity();
                        if (tList.size() > 0) {
                            if ((uEntry.material.mExtraData & 0x1) != 0) {
                                ...
                                GT_Values.RA.addElectrolyzerRecipe(GT_Utility.copyAmount(tItemAmount, stack), tCapsuleCount <= 0L ? 0 : (int) tCapsuleCount, tList.get(0), tList.size() >= 2 ? tList.get(1) : null, tList.size() >= 3 ? tList.get(2) : null, tList.size() >= 4 ? tList.get(3) : null, tList.size() >= 5 ? tList.get(4) : null, tCapsuleCount >= 0L ? tList.size() >= 6 ? tList.get(5) : null : ItemList.Cell_Empty.get(-tCapsuleCount), (int) Math.max(1L, Math.abs(uEntry.material.getProtons() * 8L * tItemAmount)), Math.min(4, tList.size()) * 30);
                            }
