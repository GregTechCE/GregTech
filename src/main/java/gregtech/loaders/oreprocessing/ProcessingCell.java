        ...
        switch (uEntry.orePrefix) {
            case cell:
                if (uEntry.material == Materials.Empty) {
                    ...
                    ModHandler.addCompressionRecipe(ItemList.Cell_Empty.get(1), GTUtility.copyAmount(1, stack));
                } else {
                    if (uEntry.material.mFuelPower > 0) {
                        GTValues.RA.addFuel(GTUtility.copyAmount(1, stack), GTUtility.getFluidForFilledItem(stack, true) == null ? GTUtility.getContainerItem(stack, true) : null, uEntry.material.mFuelPower, uEntry.material.mFuelType);
                    }
            ...
            case cellPlasma:
                if (uEntry.material == Materials.Empty) {
                    ...
                    GTValues.RA.addFuel(GTUtility.copyAmount(1, stack), GTUtility.getFluidForFilledItem(stack, true) == null ? GTUtility.getContainerItem(stack, true) : null, (int) Math.max(1024L, 1024L * uEntry.material.getMass()), 4);
                    GTValues.RA.addVacuumFreezerRecipe(GTUtility.copyAmount(1, stack), OreDictionaryUnifier.get(OrePrefix.cell, uEntry.material, 1), (int) Math.max(uEntry.material.getMass() * 2L, 1L));
                }
                break;
        }
    }
