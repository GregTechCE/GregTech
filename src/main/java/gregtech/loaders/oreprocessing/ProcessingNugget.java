...
public class ProcessingNugget implements IOreRegistrationHandler {
    ...
    public void registerOre(UnificationEntry uEntry, String modName, SimpleItemStack simpleStack) {
        ...
        if (uEntry.material == Materials.Iron)
            ModHandler.addSmeltingRecipe(GTUtility.copyAmount(1, stack), OreDictionaryUnifier.get(OrePrefix.nugget, Materials.WroughtIron, 1L));
        ...
        if (!uEntry.material.hasFlag(DustMaterial.MatFlags.NO_SMELTING)) {
            GTValues.RA.addAlloySmelterRecipe(OreDictionaryUnifier.get(OrePrefix.ingot, uEntry.material, 1L), ItemList.Shape_Mold_Nugget.get(0L), GTUtility.copyAmount(9L, stack), 100, 1);
            if ((ModHandler.getSmeltingOutput(OreDictionaryUnifier.get(OrePrefix.ingot, uEntry.material, 1), false, null) == null) && (OreDictionaryUnifier.get(OrePrefix.nugget, uEntry.material.mSmeltInto, 1L) != null) && (!ModHandler.addSmeltingRecipe(OreDictionaryUnifier.get(OrePrefix.ingot, uEntry.material, 1), GTUtility.copyAmount(9, stack)))) {
