...
public class ProcessingFoil implements IOreRegistrationHandler {
    ...
    public void registerOre(OrePrefixes aPrefix, Materials aMaterial, String aOreDictName, String aModName, ItemStack aStack) {
        if (!aMaterial.contains(SubTag.NO_SMASHING)) {
            GT_Values.RA.addBenderRecipe(GT_Utility.copyAmount(1L, OreDictionaryUnifier.get(OrePrefixes.plate, aMaterial, 4L)), OreDictionaryUnifier.get(OrePrefixes.foil, aMaterial, 4L), (int) Math.max(aMaterial.getMass(), 1L), 24);
        }
