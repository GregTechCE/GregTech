...
public class GTWorldGen_Stone extends GTWorldGen {
    ...
    public GTWorldGen_Stone(String name, boolean enabled, int minY, int maxY, int minSize, int maxSize, int probability, int amount, IBlockState stone, boolean air, String[] dimWhiteList, String[] biomeWhiteList) {
        this(name, enabled, 1024, minY, maxY, minSize, maxSize, probability, amount, stone, air, dimWhiteList, biomeWhiteList);
    }

    protected GTWorldGen_Stone(String name, boolean enabled, int sortingWeight, int minY, int maxY, int minSize, int maxSize, int probability, int amount, IBlockState stone, boolean air, String[] dimWhiteList, String[] biomeWhiteList) {
        ...
        this.amount = amount;
        this.stone = stone;
        this.stoneType = StoneType.computeStoneType(stone);
        this.air = air;
    }
        ...
        for (int i = 0; i < this.amount; i++) {
            if (random.nextInt(this.probability) == 0) {
                this.generate(random, chunkX, chunkZ, world, true,
                    ...
                    if ((this.air && blockState.getBlock().isAir(blockState, world, pos)) || StoneType.computeStoneType(blockState) != StoneTypes._NULL) {
    ...
    protected final void generate(Random random, int chunkX, int chunkZ, World world, boolean shortcut, Predicate<BlockPos> allowGenerateAtCenter, GenerateAtPos action) {
        ...
        chunkCoords/*.parallelStream()*/.forEach(chunkCoord -> {
            ...
            Random rnd = new XSTR(WorldGenerator.getRandomSeed(world, chunkCoord.x, chunkCoord.z));
            for (int x = x0; x < x1; x++) {
                for (int z = y0; z < y1; z++) {
    ...
    @FunctionalInterface
    protected interface GenerateAtPos {
